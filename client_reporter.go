...
var _ prometheus.Observer = &histogramTimer{}
...
func (t *histogramTimer) Observe(f float64) {
    t.observer.Observe(f)
}
...
func (r *clientReporter) ReceiveMessageTimer() timer {
    if r.metrics.clientStreamRecvHistogramEnabled {
        observer := r.metrics.clientStreamRecvHistogram.WithLabelValues(string(r.rpcType), r.serviceName, r.methodName)
        return newHistogramTimer(observer)
    }
    return emptyTimer
}
...
func (r *clientReporter) SendMessageTimer() timer {
    if r.metrics.clientStreamSendHistogramEnabled {
        observer := r.metrics.clientStreamSendHistogram.WithLabelValues(string(r.rpcType), r.serviceName, r.methodName)
        return newHistogramTimer(observer)
    }
